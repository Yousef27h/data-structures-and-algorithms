...
const containsAnd = (arr) => {
  let newArr = arr.filter((value)=> value.search('and') !== -1)
  return newArr;
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  let newArr = arr.filter((stat) => minBaseStat < stat.baseStat)
  return newArr;
};
...
const getStatName = (arr, minBaseStat) => {
  let newArr = arr.filter((value)=> minBaseStat < value.baseStat )
   let myArr = newArr.map((value)=> value.stat.name)
  return myArr
};
...
const getCharactersWithoutChildren = (arr) => {
  let newArr = arr.filter((value) => value.children === undefined)
  return newArr
};
