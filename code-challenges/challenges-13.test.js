...
const longestString = (arr) => {
  let longest= '';
  arr.map((string)=>{
    string.length > longest.length ? longest=string : longest=longest;
  })
...
const firstLetters = (arr) => {
  let myArr = arr.map((string) => string[0])
  return myArr
};
...
const findHappiness = (arr) => {
  let myArr = arr.filter((string) => string.includes(':)'))
  return myArr;
};
...
const standardizePhoneNumbers = (arr) => {
  let myString='';
  let myArr = arr.map((string)=>{
    myString = string.substring(1,4)+string.substring(6,9)+string.substring(10,string.length)
    return myString
  })
  return myArr;
};
