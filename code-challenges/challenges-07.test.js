...
const splitFoods = (recipe) => {
  let result = [];
  let element;
  result = recipe.ingredients.slice(0,recipe.ingredients.length).map(value => {
...
const stepActions = (recipe) => {
  let result = [];
  result = recipe.steps.slice(0,recipe.steps.length).map(value=>{
...
const removeLastCharacters = (str, numberOfCharacters) => {
  if(numberOfCharacters > str.length){
    ...
    str = str.split("")
    str.splice(str.length-(numberOfCharacters))
    str = str.join('')
    return str
  }
};
...
const totalSumCSV = (str) => {
  let total = 0;
  str = str.split(",");
  str.map((value)=> total += parseInt(value))
  return total;
};
...
const removeVowels = (str) => {
  ...
  str = str.split("");
  let arr = str.filter((value) => !vowels.includes(value.toUpperCase()))
  arr = arr.join("")
  return arr
};
...
const extractVowels = (str) => {
  ...
  arr = arr.join("");
  newArr.push(arr);
  let arr2 = str.filter((value) => vowels.includes(value.toUpperCase()))
  arr2 = arr2.sort()
  arr2 = arr2.join("");
  newArr.push(arr2)
  return newArr
